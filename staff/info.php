...
<head>
    <title>Staff - thông tin cá nhân</title>
    <!-- Required meta tags -->
    <meta charset="utf-8" />
                        ...
                        <nav class="navbar navbar-expand-lg">
                            ...
                            <div class="collapse navbar-collapse" id="navbarText">
                                ...
                                <ul class="navbar-nav float-end">
                                    <li class="nav-item">
                                        <a class="nav-link text-black fs-5" aria-current="page"
                    ...
                    <div class="mycard profile">
                        <form name="form-img" id="form-img" action="" method="post" enctype="multipart/form-data">
                            ...
                            if (isset($_FILES["upload"]["name"])) {
                                ...
                                $imageName = htmlspecialchars($_FILES["upload"]["name"], ENT_QUOTES); //tên tệp hình ảnh
                                ...
                                if (!in_array($imageExtension, $validImageExtension)) { //kiểm tra xem tên hình ảnh có hợp lệ hay không
                                    ...
                                    try {
                                        $newImageName = $name . " - " . date("Y.m.d") . " - "  . $imageExtension;
                                        $targetDirectory = '../pic/'; // Đường dẫn tới thư mục đích
                            
                                        // Cập nhật thông tin hình ảnh trong cơ sở dữ liệu
                                        $stmt1 = $conn->prepare("UPDATE users SET uImg = ? WHERE uID = ?");
                                        $stmt1->bind_param("si", $newImageName, $uid);
                                        if ($stmt1->execute()) {
                                            // Di chuyển tệp tin tạm thời đến thư mục đích
                                            if (move_uploaded_file($tmpName, $targetDirectory . $newImageName)) {
                                                echo "<script>alert('Thành công!'); console.log('" . $uid . "')</script>";
                                            } else {
                                                echo "<script>alert('Không thể di chuyển tệp tin!');</script>";
                                            }
                                        } else {
                        ...
                        <div class="profile-content px-4">
                            <form action="" class="cover" method="get" name="form-change-info" id="form-change-info">
                                <div class="mb-3 row">
                                    <label for="idstaff" class="col-sm-3 col-form-label">Mã nhân viên:</label>
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="idstaff" value="<?php echo $uid; ?>" readonly/>
                                    </div>
                                </div>
                                <div class="mb-3 row ">
                                    <label for="name" class="col-sm-3 col-form-label">Họ tên đầy đủ:</label>
                                    <div class="col-sm-8">
                                        <input type="text" class="form-control" id="namechange" value="<?php echo $name; ?>" />
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    <label for="phone" class="col-sm-3 col-form-label">Số điện thoại</label>
                                    <div class="col-sm-8">
                                        <input type="" class="form-control" id="phonechange" value="<?php echo $phone; ?>" />
                                    </div>
                                </div>
                                <div class="mb-3 row">
                            ...
                            if (isset($_POST['changePass_btn_clicked']) && ($_POST['changePass_btn_clicked']) == "2") {
                                ...
                                $stmt2 = $conn->prepare("SELECT uPass FROM users WHERE uID = ?");
                                ...
                                if ($result2->num_rows > 0) {
                                    ...
                                    $storedPassword = $row2['uPass']; // Mật khẩu đã được lưu trong cơ sở dữ liệu
                                    ...
                                    if ($oldpass === $storedPassword) {
                                        ...
                                        if ($newpass === $reEnter) {
                                            ...
                                            // Gọi procedure để thay đổi mật khẩu
                                            $stmt3 = $conn->prepare("CALL CHANGE_PASS(?, ?)");
                                            $stmt3->bind_param("is", $uid, $newpass);
                                            $stmt3->execute();
